...
def fetch_account_data(customer_id: str, refresh_token: str) -> GoogleAdsData:
    ...
    for query_name, gaql in QUERIES.items():
        try:
            ...
            # Non-fatal: store empty list and record the error in detail
            # The auditor will mark affected checks as N/A
            print(f"Warning: query '{query_name}' failed: {ex.failure.errors[0].message}")
